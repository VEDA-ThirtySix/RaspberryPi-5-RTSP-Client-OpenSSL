                           ...
                           const float fps)
{
    ...
    while (true)
    {
        ...
        if (!strcmp(method, "PLAY")) {
            ...
            strftime(filename, sizeof(filename), "%Y%m%d%H%M%S.h264", timeinfo);
            ...
            while (true) {
                ...
                if (difftime(current_time, last_save_time) >= 1.0) {
                    last_save_time = current_time;
                    char filename[256];
                    snprintf(filename, sizeof(filename), 
                             "tmp/detect%05d.yuv", file_counter++);
                    FILE *file = fopen(filename, "wb");
                    //FILE *file = fopen("../tmp/detect.yuv", "wb");
                    if (file) {
                        fwrite(capframe.y_data.data(), 1, WIDTH * HEIGHT, file);
                        fwrite(capframe.u_data.data(), 1, WIDTH * HEIGHT / 4, file);     
                        fwrite(capframe.v_data.data(), 1, WIDTH * HEIGHT / 4, file);  
                        fclose(file);
                    } else {
                        perror("Failed to save detect.yuv");
                    }
                }
